...
const AI_NAMES = ['Lauda', 'Hunt', 'Stewart', 'Fittipaldi', 'Peterson']
...
const state = {
  ...
  drivers: [
    { id: 1, name: 'Piloto 1', pace: 'normal', engine: 'normal', pushMode: 'normal', tyreWear: 0, fuel: 65, targetFuel: 65, stress: 20, fatigue: 10, pitRequested: false, selectedTyre: 'medium', lapsOnTyre: 0, lapsCompleted: 0, progress: 0, isPlayer: true, pendingOrders: {} },
    { id: 2, name: 'Piloto 2', pace: 'normal', engine: 'normal', pushMode: 'normal', tyreWear: 0, fuel: 65, targetFuel: 65, stress: 10, fatigue: 5, pitRequested: false, selectedTyre: 'medium', lapsOnTyre: 0, lapsCompleted: 0, progress: 0, isPlayer: true, pendingOrders: {} },
  ],
};
...
function bindUI() {
  ...
  driverConfigs.forEach(({ prefix, index }) => {
    document.getElementById(`${prefix}-pace`).addEventListener('change', (e) => {
      state.drivers[index].pendingOrders.pace = e.target.value;
    });
    ...
    document.getElementById(`${prefix}-fuel`).addEventListener('input', (e) => {
      ...
      state.drivers[index].pendingOrders.targetFuel = Number(val);
    });
    document.getElementById(`${prefix}-pit-request`).addEventListener('change', (e) => {
      state.drivers[index].pendingOrders.pitRequested = e.target.checked;
    });
  });
}
    ...
    if (!d.pitRequested) {
      if (tyrePct > 0.5) {
        ...
        if (roll < 0.4) {
          ...
          d.pushMode = Math.random() < 0.5 ? 'push' : 'save'
        }
      }
    }
  }
...
function getProgressGain(driver, weather) {
  ...
  return 0.5 * pace * engine * push * tyreSpeed * tyreFactor * fuelFactor * stressFactor * fatigueFactor * weatherFactor;
}

function completeLap(driver, weather) {
  Object.assign(driver, driver.pendingOrders)
